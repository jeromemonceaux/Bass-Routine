...
export default async (request, context) => {
  ...
  try {
    ...
    // - /api/library/:id
    // - /.netlify/functions/library/:id
    const match =
      pathname.match(/\/(?:api\/library|\.netlify\/functions\/library)\/?(.*)$/)
    const splat = match?.[1] || '' // empty or "<id>"

    // PATCH /.../library/:id  → upsert item in JSON array
